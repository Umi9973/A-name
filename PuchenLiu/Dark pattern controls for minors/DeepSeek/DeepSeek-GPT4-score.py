...
DOCS = [
    ...
    {
        "id": "deepseek_open_platform_terms",
        "title": "DeepSeek Open Platform Terms of Service (EN)",
...
Your task is to assess how well an official document protects minors from
...
def evaluate(doc_id, title, text):
    ...
    user_prompt = f"""
Evaluate the following official DeepSeek document for Dark Pattern Controls for Minors:

Document ID: {doc_id}
Title: {title}

The document may be truncated due to token limits.
...
def main():
    ...
    with open("deepseek_darkpattern_eval.json", "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    print("\nSaved JSON → deepseek_darkpattern_eval.json")

    with open("deepseek_darkpattern_eval.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "document_id",
            "document_title",
    ...
    print("Saved CSV → deepseek_darkpattern_eval.csv")
