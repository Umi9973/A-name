...
df_filtered = df[df["model"].isin(["gpt-4o", "ollama_mistral"])]
...
df_overall = df_overall.set_index("model").loc[["gpt-4o", "ollama_mistral"]]
...
gpt_scores = df_overall.loc["gpt-4o", labels].tolist()
ollama_scores = df_overall.loc["ollama_mistral", labels].tolist()
...
ax.plot(angles, gpt_scores, linewidth=2)
ax.fill(angles, gpt_scores, alpha=0.15)

ax.plot(angles, ollama_scores, linewidth=2)
ax.fill(angles, ollama_scores, alpha=0.15)

plt.title("GPT-4o vs Ollama (Overall Model Averages)", size=16, pad=20)
...
plt.title("GPT-4o vs Ollama — Average Scores per Prompt (including OVERALL)")
